...
import {InActiveEmployeeHeaders} from './table-headers/employees-headers';
import {data} from './table-headers/employees-headers';
import EmployeeViewDialogue from './employee-view-dialogue';
export default class  InActiveEmployessComponent extends React.Component {
    constructor(props){
        super(props);
        this.state = {
    ...
    render() {
        return ( 
            <Paper className="active-employees-container">
                <div>                    
                    <EmployeeList tableHeaders={[...InActiveEmployeeHeaders, this.viewEmployeeHeader ]} rowData={data.response} viewHandler={this.viewEmployee}/>
